...
<center>
    ...
    <div class="custom-file">
        <a href="javascript:changeProfile()" style="text-decoration: none;">
            <button type="button" class="btn btn-success">Añadir Imagen</button>
        </a>&nbsp;&nbsp;

        <a href="javascript:removeFile()" style="color: red;text-decoration: none;">
            <button type="button" class="btn btn-warning">Borrar Imagen</button>
        </a>
    </div>
    ...
    function removeFile() {
        if ($('#file_name').val() != '')
            if (confirm('¿Estas seguro de eliminar la imagen?')) {
                ...
                form_data.append('_token', '{{csrf_token()}}');
                $.ajax({
                    url: "{{asset('proveedoresImagen')}}/" + $('#file_name').val(),
                    data: form_data,
                    type: 'POST',
                    contentType: false,
                    processData: false,
